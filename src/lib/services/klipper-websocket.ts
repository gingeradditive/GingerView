			...
			try {
				...
				this.ws.onopen = () => {
					...
					toastActions.success(
						'klipper',
						'Connected',
						'WebSocket connection to Kalico established'
					);
					this.requestStatus();
					resolve();
				};
				...
				this.ws.onclose = () => {
					...
					toastActions.warning(
						'klipper',
						'Disconnected',
						'Lost connection to Kalico. Attempting to reconnect...'
					);
				...
				this.ws.onerror = (error) => {
					...
					toastActions.error(
						'klipper',
						'Connection error',
						'Failed to connect to Kalico WebSocket'
					);
					reject(error);
				};
			} catch (error) {
				...
				toastActions.error(
					'klipper',
					'Connection failed',
					'Unable to create WebSocket connection to Kalico'
				);
				reject(error);
			}
		});
	...
	private handleMessage(message: KlipperMessage): void {
		if (message.method === 'notify_status_update') {
			...
			if (status?.state === 'error' && status.state_message) {
				...
				toastActions.error(
					'klipper',
					'Printer shutdown',
					status.state_message || 'Kalico has shut down',
					0
				);
			}
		}
		...
		if (message.method === 'notify_klippy_disconnected') {
			toastActions.error(
				'klipper',
				'Klippy disconnected',
				'Kalico host process has disconnected',
				0
			);
		}
		...
		if (message.method === 'notify_klippy_shutdown') {
			toastActions.error(
				'klipper',
				'Klippy shutdown',
				'Kalico firmware has entered shutdown state',
				0
			);
		}
	...
	private attemptReconnect(): void {
		if (this.reconnectAttempts < this.maxReconnectAttempts) {
			...
			toastActions.error(
				'klipper',
				'Connection lost',
				'Max reconnection attempts reached. Unable to connect to Kalico.',
				0
			);
		}
	}
